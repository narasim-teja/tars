...
const registerOperator = async () => {
    ...
    console.log("Registering with AVS using signature:", operatorSignature);
    const tx2 = await imageVerificationServiceManager.registerOperatorToAVS(
        wallet.address,
        operatorSignature
    );
    await tx2.wait();
    console.log("Successfully registered with AVS");
};
